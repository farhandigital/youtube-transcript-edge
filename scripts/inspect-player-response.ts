...
if (!watchRes.ok) {
  console.error(`Watch page request failed: ${watchRes.status} ${watchRes.statusText}`);
  process.exit(1);
}
...
if (watchBody.includes('class="g-recaptcha"')) {
  console.error('Rate-limited by YouTube (recaptcha detected). Try again later.');
  process.exit(1);
}
...
if (!playerRes.ok) {
  console.error(`Player request failed: ${playerRes.status} ${playerRes.statusText}`);
  process.exit(1);
}
...
if (captionsOnly) {
  ...
  if (!tracklist) {
    ...
    console.log('playabilityStatus:', JSON.stringify(playerJson?.playabilityStatus, null, 2));
  } else {
...
console.log(`playabilityStatus.status : ${playerJson?.playabilityStatus?.status ?? '(missing)'}`);
...
for (const track of tracks) {
  console.log(`  → [${track.languageCode}]  baseUrl: ${track.baseUrl?.slice(0, 60)}…`);
}
